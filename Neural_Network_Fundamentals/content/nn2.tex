...
For instance if weight \(w_{11}\) is 0.6 and \(w_{21}\) is 0.4 then they are associated with an error of -0.79 multiplied by 0.6 and -0.79 multiplied by (0.4 respectively (see Pictures below).
...
\begin{itemize}
   \item the sum of errors associated with all the weights (\(w_{11}\) and \(w_{12}\) in our case) that link this node with the next layer. (see Picture above).
\end{itemize}
